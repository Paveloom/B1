...
# Блок для файла "./Make-файлы/Компиляция программ, разработка и публикация кода на GitHub"

## Массив правил для текущего make-файла
rules=( '$(main_name)' '$(obj_path)%.o' '$(obj_main)' \
        'input' 'result' 'result-r' 'result-d' 'result-c' \
        'clean' 'git' 'git-am' 'git-dev' 'git-dev-ready' \
        'git-new' 'git-new-2' 'git-dev-re' 'git-dev-ready-re' \
        'force-change' 'git-clean' )

## Путь к текущему make-файлу
path="./Make-файлы/Компиляция программ, разработка и публикация кода на GitHub"
